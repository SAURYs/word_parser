...
class check_box(QWidget):
    ...
    def check_widget(self):
        ...
        layout.addWidget(formula_continuous,2,1,Qt.AlignLeft)

        table_title = QCheckBox("检测表格标题       ")
        table_title.setChecked(True)
        measurement = QCheckBox("检测度量衡规范       ")
        measurement.setChecked(True)
        layout.addWidget(table_title,3,0,Qt.AlignLeft)
        layout.addWidget(measurement,3,1,Qt.AlignLeft)
        ...
        layout.addWidget(appendix_continuous,4,0,Qt.AlignLeft)
        layout.addWidget(references_continuous,4,1,Qt.AlignLeft)
